...
// Take full path argument
const modulePath = process.argv[2];
if (!modulePath) {
  console.error("❌ Please provide a module path. Example: node generate-module.js src/app/User");
  process.exit(1);
}

// Extract folder path and module name
const moduleName = path.basename(modulePath); // User
const targetDir = path.join(__dirname, modulePath.toLowerCase()); // src/app/user
...
// -------- Write files --------
fs.writeFileSync(path.join(targetDir, `${fileName}.model.js`), modelContent.trim());
fs.writeFileSync(path.join(targetDir, `${fileName}.repository.js`), repositoryContent.trim());
fs.writeFileSync(path.join(targetDir, `${fileName}.service.js`), serviceContent.trim());
fs.writeFileSync(path.join(targetDir, `${fileName}.controller.js`), controllerContent.trim());
fs.writeFileSync(path.join(targetDir, `${fileName}.routes.js`), routeContent.trim());

console.log(`✅ Module "${className}" generated successfully at ${targetDir}`);
